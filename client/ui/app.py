...
class AppWindow(QWidget):
    ...
    def show_page(self, name):
        ...
        if self.coffee_runtime:
            pages["coffee"] = self.coffee_runtime
        page = pages.get(name)
        if page:
    ...
    def handle_application_activated(self, result):
        instance = result.get("application_instance", {})
        if instance.get("application_id") == "coffee":
            if self.coffee_runtime:
                self.pages.removeWidget(self.coffee_runtime)
                self.coffee_runtime.deleteLater()
            self.coffee_runtime = CoffeeRuntime(instance, self)
            self.pages.addWidget(self.coffee_runtime)
            self.coffee_runtime.show_page("coffee_dashboard")
            self.show_page("coffee")
