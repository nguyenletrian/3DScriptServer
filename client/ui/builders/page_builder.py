...
from PySide6.QtWidgets import QLayout, QWidget, QLabel, QVBoxLayout, QHBoxLayout, QPushButton
...
class PageBuilder(QWidget):
    ...
    def set_error(self, message):
        if not hasattr(self, "error_label"):
            self.error_label = QLabel(); self.error_label.setStyleSheet("color: red; border: none;"); target = self.form_panel or self; layout = target if isinstance(target, QLayout) else target.layout()
            if layout: layout.addWidget(self.error_label)
    ...
    def _mutate(self, action, *args, callback=None):
        try:
            ...
            else: self.set_error(f"No handler configured for action: {action}")
        except Exception as e: self.set_error(str(e))
